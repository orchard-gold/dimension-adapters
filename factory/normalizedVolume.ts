...
async function get(slug: string, file: string, { allow404 = false } = {}) {
  ...
  if (!base) throw new Error('MARKETS_API is not set');
  return httpGet(`${base}/${slug}/${file}`).catch((e: any) => {
    const message = String(e?.message ?? 'request failed').split(base).join('<markets-api>');
